...
- Remote publisher text is sent to Claude Opus 4 as untrusted JSON evidence. The model supplies only Message Pull-Through and Strategic Value, which are rejected if they add unsupported audience or positioning claims. Performance / Reach, headline, URL, links, quote, metrics, and labels are rendered deterministically from verified data.
